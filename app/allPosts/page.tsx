                    ...
                    <table className="w-full border-collapse">
                        <thead className="bg-gray-100">
                            <tr className="text-gray-600 text-sm font-medium">
                                <th className="py-3 px-4 text-left border-r border-gray-200">Source</th>
                                <th className="py-3 px-4 text-left border-r border-gray-200">Username</th>
                                <th className="py-3 px-4 text-left border-r border-gray-200">Saved Date</th>
                                <th className="py-3 px-4 text-left border-r border-gray-200">Content</th>
                                <th className="py-3 px-4 text-left border-r border-gray-200">Severity Index</th>
                                <th className="py-3 px-4 text-left border-r border-gray-200">Suggested Outreach</th>
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={7} className="text-center py-8 text-gray-500">No results found.</td>
                                </tr>
                            ) : (
                                data.map((row, index) => (
                                    <tr key={row.id} className={`${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-gray-100 transition-colors`}>
                                            ...
                                            <div className="flex flex-col space-y-2">
                                                <div className="flex items-center gap-2">
                                                    <span className={`px-2 py-1 rounded text-xs font-medium text-white ${
                                                        row.source === 'discord' ? 'bg-indigo-500' : 'bg-orange-500'
                                                    }`}>
                                                        {row.source}
                                                    </span>
                                                </div>
                                        ...
                                        <td className="py-4 px-4 border-r border-gray-200">
                                            <span className={`inline-flex items-center justify-center w-10 h-6 rounded text-white text-sm font-bold ${
                                                row.score <= -4 ? 'bg-red-500' :
                                                row.score <= -3 ? 'bg-orange-500' :
                                                row.score <= -2 ? 'bg-yellow-500' :
                                            ...
                                            <div className="bg-blue-50 border-l-4 border-blue-400 p-3 rounded">
                                                <div className="text-sm text-gray-700 mb-3">
                                                    {expandedRows[row.id] ? 
                                                        ...
                                                        row.suggested_outreach?.slice(0, 100) + (row.suggested_outreach?.length > 100 ? '...' : '')
                                                    }
                                                </div>
                                                <div className="flex items-center gap-3 text-xs">
                                                    <button 
                                                        className="flex items-center gap-1 text-gray-600 hover:text-gray-800"
                                                        onClick={() => copyToClipboard(row.suggested_outreach)}
                                                    >
                                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                                                        </svg>
